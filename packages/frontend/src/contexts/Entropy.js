...
export default class Entropy {
  ...
  generateEntropy() {
    ...
    const entropyAsBytes = new Uint8Array(
      generatedEntropyAsBytes.length + randomEntropyAsBytes.length
    )
    entropyAsBytes.set(generatedEntropyAsBytes)
    entropyAsBytes.set(randomEntropyAsBytes, generatedEntropyAsBytes.length)
    /*
